...
export class RiskIngestionService {
    static async ingestRisk(data) {
        ...
        if (!data.category || data.severity === undefined || !data.source) {
    ...
    static async getUnprocessedRisks(limit = 5) {
        await connectToDatabase();
        return RiskInput.find({ processed: false }).sort({ timestamp: 1 }).limit(limit);
    }
